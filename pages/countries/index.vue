<template>
  <main class="">
    ...
    <ThemeTable
      ...
      :items="data"
      :meta="meta"
      :loading="useCountriesStore().loading"
    ...
    <CountriesCreate v-if="$modal.value == 'CreateItem'" />
    <ThemePlusButton
      @click.native="
...
await useCountriesStore().fetchData();
loading.value = false;
const data = computed(() => useCountriesStore().data);
const meta = computed(() => useCountriesStore().meta);
...
async function activeHandler(id: number, value: any) {
  useCountriesStore().loading = true;
  await $http(`/admin/course/activation/${id}?active=${value}`);
  useCountriesStore().loading = false;
  useCountriesStore().fetchData();
