<template>
  <main class="">
    <ThemeTitle class="mb-6" text="انواع المعاملة" />

    <ThemeTable
      :headers="headers"
      :items="data"
      :meta="meta"
      :loading="useProcessTypesStore().loading"
    ...
    <ProcessTypesCreate v-if="$modal.value == 'CreateItem'" />
    <ThemePlusButton
      @click.native="
...
await useProcessTypesStore().fetchData();
loading.value = false;
const data = computed(() => useProcessTypesStore().data);
const meta = computed(() => useProcessTypesStore().meta);
...
async function activeHandler(id: number, value: any) {
  useProcessTypesStore().loading = true;
  await $http(`/admin/course/activation/${id}?active=${value}`);
  useProcessTypesStore().loading = false;
  useProcessTypesStore().fetchData();
