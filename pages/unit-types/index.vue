<template>
  <main class="">
    <ThemeTitle class="mb-6" text="انواع الوحدة" />

    <ThemeTable
      :headers="headers"
      :items="data"
      :meta="meta"
      :loading="useUnitTypesStore().loading"
    ...
    <UnitTypesCreate v-if="$modal.value == 'CreateItem'" />
    <ThemePlusButton
      @click.native="
...
await useUnitTypesStore().fetchData();
loading.value = false;
const data = computed(() => useUnitTypesStore().data);
const meta = computed(() => useUnitTypesStore().meta);
...
async function activeHandler(id: number, value: any) {
  useUnitTypesStore().loading = true;
  await $http(`/admin/course/activation/${id}?active=${value}`);
  useUnitTypesStore().loading = false;
  useUnitTypesStore().fetchData();
